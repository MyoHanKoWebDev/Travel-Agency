        ...
        <div class="container mt-5 mt-md-0">
            ...
            <div class="row g-4 justify-content-center">
                        ...
                        <div class="col-lg-4 col-md-6 rounded-2 wow fadeInUp" data-wow-delay="<?php echo $wonDelay . 's'; ?>">
                            <div class="package-item">
                                <div class="img-con overflow-hidden">
                                    ...
                                    <?php
                                    //check full book or not
                                    $pgId = $row['pgID'];
                                    $checkFullBook = "SELECT SUM(bk.bkPQ) AS totalPQ, p.numOfPeo  FROM bookingDetail bk
                                                        LEFT JOIN booking b ON bk.bID = b.bID
                                                        LEFT JOIN package p ON p.pgID = bk.pgID
                                                        WHERE bk.pgID = $pgId and b.bstatus= 'Confirmed'";
                                    $resultAv = mysqli_query($con, $checkFullBook);
                                    if ($resultAv) {
                                                $rowAv = mysqli_fetch_assoc($resultAv);
                                                $remainingSeats = $row["numOfPeo"] - $rowAv["totalPQ"];
                                                if ($remainingSeats == 0) {
                                    ?>
                                                    <div class="ribbon ribbon-top-left text-center"><span class="bg-white text-danger fs-6">Fully Booked</span></div>
                                    <?php } }
                                        ...
                                        if (mysqli_num_rows($reSt) > 0) {
                                            ...
                                            while ($rowSt = mysqli_fetch_assoc($reSt)) {
                                                ...
                                                if ($rowSt['bstatus'] == 'Pending') {
                                                    $status = "Pending";
                                                ?>
                                                    <div class="ribbon ribbon-top-left text-center">
                                                        <span class="bg-warning text-white"><?php echo $rowSt["bstatus"]; ?></span></div>
                                                <?php       } else if ($rowSt['bstatus'] == 'Confirmed') {
                                                    $status = "Confirmed";
                                                ?>
                                                    <div class="ribbon ribbon-top-left text-center">
                                                        <span class="bg-primary text-white"><?php echo $rowSt["bstatus"]; ?></span></div>
                                                <?php       } else if ($rowSt['bstatus'] == 'Rejected') {
                                                    $status = "Rejected";
                                                ?>
                                                    <div class="ribbon ribbon-top-left text-center">
                                                        <span class="bg-danger text-white"><?php echo $rowSt["bstatus"]; ?></span></div>
                                    <?php }
                                ...
                                <div class="text-center p-4">
                                    ...
                                    <div class="d-flex justify-content-center mb-2">
                                        <a href="readMorePack.php?reMorePg=<?php echo $row["pgID"]; ?>" class="btn btn-sm btn-primary px-5 py-2 border-end" 
                ...
                if ($totalPages > 1) {
                    ...
                    // Previous Button
                    if ($page > 1) {
                        echo '<li class="page-item me-2"><a class="page-link rounded-circle px-3 py-2 border-primary" href="?page=' . ($page - 1) . '
                        " aria-label="Previous"><span class="fw-bold" aria-hidden="true">&laquo;</span></a></li>';
                    }

                    // Page Number Links
                    for ($i = 1; $i <= $totalPages; $i++) {
                        $activeClass = ($i == $page) ? 'active' : '';
                        echo '<li class="page-item  ' . $activeClass . ' me-2">
                        <a class="page-link rounded-circle px-3 py-2 border-primary " href="?page=' . $i . '">' . $i . '</a></li>';
                    }

                    // Next Button
                    if ($page < $totalPages) {
                        echo '<li class="page-item"><a class="page-link rounded-circle px-3 py-2 border-primary" href="?page=' . ($page + 1) . '
                        " aria-label="Next"><span class="fw-bold" aria-hidden="true">&raquo;</span></a></li>';
                    }
