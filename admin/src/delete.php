...
if (isset($_GET["dePy"])) {
    ...
    $sqlChPay = "SELECT * FROM booking WHERE pyID = $dePayId";
...
if (isset($_GET["dePg"])) {
    ...
    $sqlChPg = "SELECT * FROM bookingdetail WHERE pgID = $dePgId";
    $res = mysqli_query($con, $sqlChPg);
    if (mysqli_num_rows($res) > 0) {
        echo "<script>alert('Cannot delete this package because it is linked to booking systems.');
              window.location.replace('viewPack.php');</script>";
        exit();
    } else {
                echo "<script>
                if (confirm('Do you really want to delete this car?')) {
                    window.location.href = 'delete.php?confirmPgDe=$dePgId';
                } else {
                    window.location.replace('viewPack.php');
                }
              </script>";
        exit(); 
    }
...
if (isset($_GET["confirmPgDe"])) {
    $confirmDePgId = $_GET["confirmPgDe"];
    $sqlDePg = "DELETE FROM package WHERE pgID = $confirmDePgId";
